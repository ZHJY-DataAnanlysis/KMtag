...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph_type='GAT', layer=1, path_list=None, data_path=None):
        ...
        # 使用实际的标签字典长度而不是硬编码
        self.label_num = len(torch.load(os.path.join(data_path, 'value_dict.pt'), weights_only=False))
        self.graph_type = graph_type
    ...
    def forward(self, label_emb, embeddings):
        ...
        if self.graph_type == 'graphormer':
            ...
            # 将path_list转换为PyTorch Geometric需要的edge_index格式
            if self.path_list is not None and len(self.path_list) > 0:
                # path_list是(node_id, label_id)对的列表，需要转换为edge_index
                edges = []
                try:
                    for edge in self.path_list:
                        if isinstance(edge, (list, tuple)) and len(edge) >= 2:
                            node_id, label_id = edge[0], edge[1]
                            # 确保索引在有效范围内
                            if node_id < self.label_num and label_id < self.label_num:
                                edges.append([node_id, label_id])
                                # 添加反向边以创建无向图
                                if node_id != label_id:
                                    edges.append([label_id, node_id])
                except (ValueError, TypeError) as e:
                    print(f"Error processing path_list: {e}, path_list type: {type(self.path_list)}")
                    print(f"First few elements: {self.path_list[:5] if len(self.path_list) > 0 else 'empty'}")
                    # 如果解析失败，创建自环
                    edges = [[i, i] for i in range(self.label_num)]
                
                if edges:
                    extra_attn = torch.tensor(edges, dtype=torch.long).t().contiguous()
                    # 确保edge_index在正确的设备上
                    extra_attn = extra_attn.to(label_emb.device)
                else:
                    # 如果没有有效边，创建自环
                    extra_attn = torch.arange(self.label_num).repeat(2, 1).to(label_emb.device)
            else:
                # 如果path_list为空，创建自环
                extra_attn = torch.arange(self.label_num).repeat(2, 1).to(label_emb.device)
