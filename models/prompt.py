...
class Prompt(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # 检查是否有多分类位置
            if multiclass_pos.any():
                # 获取多分类位置的掩码
                multiclass_mask = multiclass_pos.unsqueeze(-1).expand(-1, -1, prediction_scores.size(-1))
                multiclass_logits = prediction_scores.masked_select(multiclass_mask).view(-1,
                ...
                # 正确提取multiclass对应的标签
                # multiclass_logits的形状是 [num_multiclass_positions, num_labels] 
                # labels的形状是 [batch_size, max_depth * num_class]
                # 我们需要重构标签以匹配logits的维度
                
                batch_size, seq_len = multiclass_pos.shape
                
                # 找到有multiclass token的样本
                multiclass_samples = []
                for batch_idx in range(batch_size):
                    if multiclass_pos[batch_idx].any():
                        multiclass_samples.append(batch_idx)
                
                if multiclass_samples and len(multiclass_samples) == multiclass_logits.size(0):
                    # 计算max_depth
                    total_label_size = labels.size(-1)  # max_depth * num_class
                    if self.num_labels == 0:
                        multiclass_loss = torch.tensor(0.0, device=labels.device, requires_grad=True)
                    else:
                        max_depth = total_label_size // self.num_labels
                        if total_label_size % self.num_labels != 0:
                            usable_size = (total_label_size // self.num_labels) * self.num_labels
                            max_depth = usable_size // self.num_labels
                        
                        # 提取标签并重构维度
                        batch_labels = labels[multiclass_samples]
                        
                        # 截取前面可用的部分
                        if total_label_size % self.num_labels != 0:
                            batch_labels = batch_labels[:, :max_depth * self.num_labels]
                        
                        # 重构为 [num_samples, max_depth, num_class]
                        reshaped_labels = batch_labels.view(len(multiclass_samples), max_depth, self.num_labels)
                        
                        # 合并标签：处理-100值
                        merged_labels = torch.full((len(multiclass_samples), self.num_labels), -100, 
                                                  dtype=reshaped_labels.dtype, device=reshaped_labels.device)
                        
                        for sample_idx in range(len(multiclass_samples)):
                            for class_idx in range(self.num_labels):
                                class_values = reshaped_labels[sample_idx, :, class_idx]
                                valid_values = class_values[class_values != -100]
                                
                                if len(valid_values) > 0:
                                    merged_labels[sample_idx, class_idx] = valid_values.max()
                        
                        # 计算损失
                        if merged_labels.size(-1) == multiclass_logits.size(-1):
                            valid_mask = merged_labels != -100
                            
                            if valid_mask.any():
                                loss_fn = nn.BCEWithLogitsLoss(reduction='none')
                                per_element_loss = loss_fn(multiclass_logits, merged_labels.float())
                                valid_loss = per_element_loss * valid_mask.float()
                                multiclass_loss = valid_loss.sum() / valid_mask.sum().float()
                            else:
                                multiclass_loss = torch.tensor(0.0, device=labels.device, requires_grad=True)
                        else:
                            multiclass_loss = torch.tensor(0.0, device=labels.device, requires_grad=True)
    ...
    @torch.no_grad()
    def generate(self, input_ids, depth2label, threshold=0.01, **kwargs):
        ...
        if multiclass_pos.any():
            ...
            # 如果没有多分类位置，返回零向量
            prediction_scores = torch.zeros(1, self.num_labels, device=input_ids.device)
        
        # 应用sigmoid激活函数
        prediction_scores = torch.sigmoid(prediction_scores)

        # 修复：安全地处理prediction_scores的reshape
        batch_size = input_ids.size(0)
        num_depths = len(depth2label)

        # 检查prediction_scores的总元素数是否能安全重塑
        total_elements = prediction_scores.numel()
        expected_elements = batch_size * num_depths * prediction_scores.size(-1)

        if total_elements != expected_elements:
            # 维度不匹配时，将每个样本的预测分数复制到所有深度
            num_labels = prediction_scores.size(-1)
            new_prediction_scores = torch.zeros(batch_size, num_depths, num_labels,
                                                device=prediction_scores.device)
            
            # 为每个样本复制预测分数到所有深度
            samples_with_predictions = min(prediction_scores.size(0), batch_size)
            for sample_idx in range(samples_with_predictions):
                for depth_idx in range(num_depths):
                    new_prediction_scores[sample_idx, depth_idx] = prediction_scores[sample_idx]
            
            prediction_scores = new_prediction_scores
        else:
            # 正常情况下重塑
            prediction_scores = prediction_scores.view(batch_size, num_depths, prediction_scores.size(-1))

        predict_labels = []
        for batch_idx in range(batch_size):
            batch_predictions = []
            for depth_idx in range(num_depths):
                score = prediction_scores[batch_idx, depth_idx]
                for label_idx in depth2label[depth_idx]:
                    if label_idx < score.size(0) and score[label_idx] > threshold:
                        batch_predictions.append(label_idx)
            predict_labels.append(batch_predictions)
