...
if __name__ == '__main__':
    ...
    if 'path_list' in checkpoint and checkpoint['path_list'] is not None:
        path_list = checkpoint['path_list']
        print(f"Using path_list from checkpoint: length={len(path_list)}")
    else:
        path_list = [(i, v) for v, i in value2slot.items()]
        print(f"Using computed path_list: length={len(path_list)}")
    ...
    if 'path_list' in checkpoint:
        saved_path_list = checkpoint['path_list']
        print(f"Using path_list from checkpoint with shape: {saved_path_list.shape if hasattr(saved_path_list, 'shape') else len(saved_path_list)}")
    else:
        saved_path_list = path_list
        print(f"Using computed path_list with length: {len(path_list)}")
    
    model = Prompt.from_pretrained(args.arch, num_labels=num_class, path_list=saved_path_list, layer=args.layer,
